...
void test_concurrency() {
    ...
    if (size() != num_threads * num_items_per_thread) {
        count_failed++;
    }
    ...
    if (counter != num_threads * num_items_per_thread) {
        count_failed++;
    }
...
void stress_test() {
    ...
    if (enqueue_counter != (num_threads / 2) * num_operations || dequeue_counter != (num_threads / 2) * num_operations) {
        count_failed++;
    }
...
//gcc -O3 -D_POSIX_C_SOURCE=200809 -Wall -std=c11 -pthread queue.c test1.c -o test1